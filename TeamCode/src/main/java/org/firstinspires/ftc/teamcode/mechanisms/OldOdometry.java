...
import org.firstinspires.ftc.teamcode.util.OldLocation;
 ...
 */
public class OldOdometry extends Mechanism {
...
//503.48
    ...
    private OldLocation position = new OldLocation();
    /*Declares another new Location object to track position*/
    public OldLocation realMaybe = new OldLocation();
     ...
     */
    public OldOdometry(HardwareMap hardwareMap) {
     ...
     * Odometry Constructor
     */
    public OldOdometry(HardwareMap hardwareMap, OldLocation startPos) {
    ...
    public OldOdometry(HardwareMap hardwareMap, double distance, double centerDistance, OldLocation startingLocation) {
        expansionHub = hardwareMap.get(ExpansionHubEx.class, "Expansion Hub 1");
     ...
     * Odometry constructor
     */
    public OldOdometry(HardwareMap hardwareMap, double distance, double centerDistance) {
        expansionHub = hardwareMap.get(ExpansionHubEx.class, "Expansion Hub 1");
        reset();
    ...
    public void reset(float x, float z, float rot) {
        try {
            ...
            OldLocation resettiSpot = new OldLocation(x, 0, z, rot);
            encoderPosition = new int[3];
     ...
     * in, or, in the case of no arguments, to (0, 0, 0), also resets the encoder positions.
     */
    public void reset(OldLocation resetPos) {
        try {
            bulkData = expansionHub.getBulkInputData();
            for (int i = 0; i < 3; i++) {
                if (i == 2) {
            ...
            position = resetPos;
            realMaybe=new OldLocation(resetPos.getLocation(2),0,resetPos.getLocation(0),resetPos.getLocation(3));
    ...
    //Gets the position
    public OldLocation getPosition() {
        try {return position;}
        catch (NullPointerException e) {
            return new OldLocation();
        }
    }
